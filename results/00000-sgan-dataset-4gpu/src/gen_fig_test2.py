...
synthesis_kwargs = dict(output_transform=dict(func=tflib.convert_images_to_uint8, nchw_to_nhwc=True), 
    minibatch_size=16, truncation_psi = 1.2)
...
def load_Gs(url):
    ...
    #    _Gs_cache[url] = Gs
    _G, _D, Gs = pickle.load(open("results/00002-sgan-dataset-4gpu/network-snapshot-038341.pkl", "rb"))
...
def main():
    ...
    draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing.png'), load_Gs(url_ffhq), w=256, h=256, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,14)])
    ...
    draw_truncation_trick_figure(os.path.join(config.result_dir, 'figure08-truncation-trick.png'), load_Gs(url_ffhq), w=256, h=256, seeds=[91,388, 389, 390, 391, 392, 393, 394, 395, 396], psis=[5, 0.75, 0.5, 0.25, 0, -0.25, -0.5, -0.75, -1])
